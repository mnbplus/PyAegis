...
def test_scan_code_string_detects_injection():
    ...
    assert any("os.system" in s for s in sinks), (
...
def test_scan_code_string_respects_sanitizer():
    ...
    assert len(findings) == 0, (
        f"Expected no findings for clean code, got: {findings}"
    )
...
def test_scan_file():
    ...
    try:
        ...
        assert isinstance(findings, list)
        assert len(findings) >= 1, (
            f"Expected findings from scan_file, got: {findings}"
        )
    finally:
        os.unlink(tmp_path)
...
def test_severity_filter():
    ...
    all_findings = scan_code_string(_INJECTION_CODE, return_format="dict")
